...
} from '../auth/authenticated-username.repository';
import { localForageStore } from './local-forage';

type PropsFactories = [PropsFactory<any, any>, ...PropsFactory<any, any>[]];

const storeMapByName = new Map<string, Store>();
const storeInitializedMapByName = new Map<string, Observable<boolean>>();
const storePropsFactoriesMapByName = new Map<string, PropsFactories>();

const buildStoreNameWithUsername = (storeName: string, username: string) =>
  `${storeName}:${username || 'anonymous'}`;
...
const createUserStoreWithPersistence = (
  ...
  const store = createStore({ name: storeNameWithUsername }, ...propsFactories);

  storePropsFactoriesMapByName.set(rawStoreName, propsFactories);
  storeMapByName.set(storeNameWithUsername, store);

  storeInitializedMapByName.set(
    storeNameWithUsername,
    persistState(store, {
...
export const isUserStoreInitialized$ = (rawStoreName: string): Observable<boolean> => {
  const storeNameWithUsername = buildStoreNameWithUsername(
    rawStoreName,
    getAuthenticatedUsername(),
  );
...
const selectUserStore = (name: string): Observable<Store> =>
  authenticatedUsername$.pipe(map((username) => getOrCreateUserStore(name, username)));

const getUserStore = (name: string): Store =>
  getOrCreateUserStore(name, getAuthenticatedUsername());
...
export const registerUserStore = (
  ...
  if (!storeMapByName.has(storeNameWithUsername)) {
    createUserStoreWithPersistence(storeConfig.name, username, ...propsFactories);
  }

  return [selectUserStore(storeConfig.name), () => getUserStore(storeConfig.name)];
};

...
export class ScheduleListService {
  ...
  eventsToEventsByDay(events: Event[], dateInterval: Interval): EventsByDay[] {
    ...
    const eventsMapByDay = events.reduce((accumulatorEventsMapByDay, event) => {
    ...
    return days
      ...
      .map((day) => ({
        day,
        events: eventsMapByDay.has(day) ? eventsMapByDay.get(day) : [],
      }));
  }
  ...
  loadEventsByDays(
    viewStartDate,
    endDateToLoad,
    outOfStateSchedule?: Schedule,
  ): Observable<EventsByDay[]> {
    const intervalViewForStateEvents = {
      start: viewStartDate,
      end: this.scheduleService.getStateEndDate(),
    };
    ...
    if (isAfter(endDateToLoad, this.scheduleService.getStateEndDate())) {
      ...
      return this.scheduleService.getStoreManager().displayedEvents$.pipe(
        ...
        switchMap((stateEventsByDay) =>
          this.scheduleService.outOfStateScheduleToDisplayedEvents(outOfStateSchedule).pipe(
            map((events) => {
